...
-- =====================================================================
-- =====================================================================
--  SMOOTH BUILD SUPPORT (anti nge-frame)
-- =====================================================================
local ContentProvider = game:GetService("ContentProvider")
local PRELOAD        = {}       -- semua image yang dipakai UI (buat PreloadAsync)
local BUILD_BUDGET   = 0.004    -- max kerja per frame saat build (4ms)
local _lastYield     = os.clock()
local BUILDING       = false    -- true selama UI masih di-build (mode Deferred)

-- yield HANYA kalau budget frame kelewat -> kerja kesebar antar frame,
-- jadi game tetep jalan (ga freeze) tapi build tetep secepat mungkin.
local function budgetYield(force)
	if force or (os.clock() - _lastYield) >= BUILD_BUDGET then
		task.wait()
		_lastYield = os.clock()
	end
end
...
local function create(class, props, children)
	...
	if props and type(props.Image) == "string" and props.Image ~= "" then
		PRELOAD[#PRELOAD + 1] = props.Image
	end
	if props then
...
local function registerElement(id, handle)
	if id then
		-- snapshot nilai default saat element baru dibuat (buat ResetConfig)
		if handle and handle.Get and handle._default == nil then
			local ok, v = pcall(handle.Get)
			if ok then handle._default = v end
		end
...
function Vypers:LoadConfig(name)
	...
	self._silentNotify = true   -- bisukan notif selama replay callback
...
-- true kalau library lagi nge-load/replay config (buat guard notif custom lu sendiri)
function Vypers:IsLoadingConfig()
	return self._silentNotify == true
end
...
-- =====================================================================
--  SHAREABLE CONFIG (JSON string buat copy-paste antar user)
-- =====================================================================

-- Kumpulkan semua nilai element -> JSON string
function Vypers:GetConfigJSON()
	local data = {}
	for id, el in pairs(self._elements) do
		if el.Get then
			local ok, val = pcall(el.Get)
...
-- Terima JSON string -> apply ke element + fire callback
function Vypers:LoadConfigFromJSON(json)
...
-- Kembalikan semua element ke nilai default-nya
function Vypers:ResetConfig()
	self._silentNotify = true   -- bisukan notif selama replay callback
	for id, el in pairs(self._elements) do
		if el and el.Set and el._default ~= nil then
			pcall(el.Set, el._default)
			if el.Callback then
				local arg = el._default
				if el.Get then
					local okg, g = pcall(el.Get)
					if okg then arg = g end
				end
...
-- Hapus config tersimpan (file executor / memory). Return true, removed(boolean)
function Vypers:DeleteConfig(name)
	name = name or self._configName or "default"
	local removed = false
	if hasFileAPI and typeof(delfile) == "function" and safeIsFile(self:_configPath(name)) then
		local ok = pcall(delfile, self:_configPath(name))
		removed = removed or ok
	end
	if self._memoryConfig and self._memoryConfig[name] ~= nil then
		self._memoryConfig[name] = nil
		removed = true
	end
	return true, removed
end
...
-- SetTheme: override warna apapun sebelum CreateWindow.
-- Contoh: Vypers:SetTheme({ Surface = Color3.fromRGB(20,20,30), SurfaceLight = Color3.fromRGB(40,40,55) })
function Vypers:SetTheme(tbl)
	if type(tbl) == "table" then
		for k, v in pairs(tbl) do self._theme[k] = v end
	end
	return self._theme
end
...
function Vypers:Notify(opts)
	opts = opts or {}
	-- lagi replay config (load/reset)? jangan tampilkan toast biar gak kespam pas execute.
	if self._silentNotify then return end
...
-- =====================================================================
--  LOADING SCREEN  (kartu progress clean di pojok kanan bawah)
-- ---------------------------------------------------------------------
--  local Loader = Vypers:CreateLoadingScreen({ Title=, SubTitle=, Accent=, Width= })
--    Loader:Set(alpha, label)   -- alpha 0..1, bar nge-tween halus + update teks
--    Loader:Add(delta, label)   -- nambah progress relatif
--    Loader:Get()               -- baca progress sekarang (0..1)
--    Loader:Finish(callback)    -- isi ke 100%, fade out, lalu jalankan callback
--    Loader:Destroy()           -- buang paksa
-- =====================================================================
function Vypers:CreateLoadingScreen(opts)
	opts = opts or {}
	local theme  = self._theme
	local accent = opts.Accent or theme.Accent
	local title  = opts.Title or "Loading"
	local sub    = opts.SubTitle or "Preparing interface..."
	local width  = opts.Width or 258
	local gui    = self:_ensureGui()
	local TweenService = game:GetService("TweenService")

	-- kartu utama (nempel di pojok kanan bawah)
	local card = create("Frame", {
		Name = "LoadingScreen", Parent = gui,
		BackgroundColor3 = theme.Surface, BackgroundTransparency = 1, BorderSizePixel = 0,
		AnchorPoint = Vector2.new(1, 1), Position = UDim2.new(1, -20, 1, -20),
		Size = UDim2.fromOffset(width, 76), ZIndex = 900,
	})
	corner(card, RADIUS_LARGE)
	local cardStroke = stroke(card, theme.Border, 1)
	cardStroke.Transparency = 1
	create("UIPadding", { Parent = card,
		PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 12),
		PaddingLeft = UDim.new(0, 14), PaddingRight = UDim.new(0, 14) })

	-- baris judul + persen
	local titleLabel = create("TextLabel", {
		Parent = card, BackgroundTransparency = 1, Font = FONT_TITLE, Text = title,
		TextColor3 = theme.Text, TextSize = 14, TextTransparency = 1,
		TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd,
		Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(1, -42, 0, 16), ZIndex = 901,
	})
	local pctLabel = create("TextLabel", {
		Parent = card, BackgroundTransparency = 1, Font = FONT_TITLE, Text = "0%",
		TextColor3 = accent, TextSize = 13, TextTransparency = 1,
		TextXAlignment = Enum.TextXAlignment.Right,
		Position = UDim2.new(1, -42, 0, 0), Size = UDim2.new(0, 42, 0, 16), ZIndex = 901,
	})
	local subLabel = create("TextLabel", {
		Parent = card, BackgroundTransparency = 1, Font = FONT_VALUE, Text = sub,
		TextColor3 = theme.TextDim, TextSize = 11, TextTransparency = 1,
		TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd,
		Position = UDim2.new(0, 0, 0, 19), Size = UDim2.new(1, 0, 0, 14), ZIndex = 901,
	})

	-- track + fill (bar progress)
	local track = create("Frame", {
		Parent = card, BackgroundColor3 = theme.SurfaceLight, BackgroundTransparency = 1,
		BorderSizePixel = 0, AnchorPoint = Vector2.new(0, 1),
		Position = UDim2.new(0, 0, 1, 0), Size = UDim2.new(1, 0, 0, 6), ZIndex = 901,
	})
	corner(track, 3)
	local fill = create("Frame", {
		Parent = track, BackgroundColor3 = accent, BorderSizePixel = 0,
		Size = UDim2.new(0, 0, 1, 0), ZIndex = 902,
	})
	corner(fill, 3)

	-- entrance: fade masuk halus
	local ein = TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	TweenService:Create(card, ein, { BackgroundTransparency = 0 }):Play()
	pcall(function() TweenService:Create(cardStroke, ein, { Transparency = 0 }):Play() end)
	TweenService:Create(titleLabel, ein, { TextTransparency = 0 }):Play()
	TweenService:Create(pctLabel, ein, { TextTransparency = 0 }):Play()
	TweenService:Create(subLabel, ein, { TextTransparency = 0 }):Play()
	TweenService:Create(track, ein, { BackgroundTransparency = 0 }):Play()

	local handle = {}
	local alpha  = 0
	local dead   = false

	local function apply(a, animate)
		a = clamp(a, 0, 1)
		alpha = a
		pctLabel.Text = math.floor(a * 100 + 0.5) .. "%"
		if animate == false then
			fill.Size = UDim2.new(a, 0, 1, 0)
		else
			TweenService:Create(fill, TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
				{ Size = UDim2.new(a, 0, 1, 0) }):Play()
		end
	end

	function handle:Set(a, label)
		if dead then return handle end
		if label ~= nil then subLabel.Text = tostring(label) end
		apply(tonumber(a) or 0, true)
		return handle
	end
	function handle:Add(delta, label) return handle:Set(alpha + (tonumber(delta) or 0), label) end
	function handle:Get() return alpha end
	function handle:Destroy()
		if dead then return end
		dead = true
		if card then card:Destroy() end
	end
	function handle:Finish(callback)
		if dead then if callback then task.spawn(callback) end return handle end
		apply(1, true)
		subLabel.Text = opts.DoneText or "Ready"
		task.delay(0.30, function()
			...
			for _, d in ipairs(card:GetDescendants()) do
				pcall(function()
					...
					if d:IsA("Frame") then TweenService:Create(d, fade, { BackgroundTransparency = 1 }):Play() end
					if d:IsA("UIStroke") then TweenService:Create(d, fade, { Transparency = 1 }):Play() end
				end)
			end
			local slide = TweenService:Create(card, fade, { Position = UDim2.new(1, -20, 1, 12) })
			slide:Play()
...
function Vypers:CreateWindow(opts)
	...
	local deferred = (opts.Deferred == true) or (opts.StartHidden == true)
	if deferred then
		-- window di-build DI LUAR ScreenGui (Parent = nil) biar Roblox ga ngitung
		-- layout/render tiap element yang dibikin. Di-parent pas Window:Show().
		BUILDING   = true
		_lastYield = os.clock()
	end

	-- === TRANSPARANSI ELEMENT (0..1) — semua bisa di-set dari example ===
	-- 0 = solid penuh (nutupin gambar), 1 = ilang total (cuma border/teks).
	-- Default nyala pas Transparent=true. (Di Lua 0 itu truthy, jadi nilai 0 dihormati.)
	local glass = (opts.Transparent ~= false)
	local function pickT(v, def)
		if v ~= nil then return v else return def end
	end
	self._surfaceT = pickT(opts.SurfaceTransparency, glass and 0.5 or 0)  -- card tiap element (slider/toggle/dll)
	self._sectionT = pickT(opts.SectionTransparency, glass and 0.85 or 0) -- panel/box section (biar ga numpuk sama card)
	self._tabT     = pickT(opts.TabTransparency, glass and 0.4 or 0)      -- tombol tab di sidebar

	-- === WARNA BACKGROUND ITEM (bisa di-set dari example) ===
	-- Cara 1 (alias praktis):
	--   opts.ItemColor    -> warna card tiap element (slider/toggle/button/input/track/dll)
	--   opts.SectionColor -> warna panel/box section + title bar
	--   opts.TabColor     -> warna tombol tab di sidebar (state normal)
	--   opts.WindowColor  -> warna dasar window
	--   opts.Accent       -> warna aksen (tab aktif, dll)
	-- Cara 2 (override penuh): opts.Theme = { Surface=, SurfaceLight=, SurfaceHover=,
	--   Border=, Accent=, Text=, TextDim=, TextMuted=, ToggleOff=, Background=, ... }
	do
		local newTheme = {}
		for k, v in pairs(self._theme) do newTheme[k] = v end
		if type(opts.Theme) == "table" then
			for k, v in pairs(opts.Theme) do newTheme[k] = v end
		end
		if opts.ItemColor    ~= nil then newTheme.SurfaceLight = opts.ItemColor end
		if opts.ElementColor ~= nil then newTheme.SurfaceLight = opts.ElementColor end
		if opts.SectionColor ~= nil then newTheme.Surface      = opts.SectionColor end
		if opts.TabColor     ~= nil then newTheme.SurfaceHover  = opts.TabColor end
		if opts.WindowColor  ~= nil then newTheme.Background    = opts.WindowColor end
		if opts.Accent       ~= nil then newTheme.Accent        = opts.Accent end
		self._theme = newTheme -- scoped ke window ini, ga ganggu THEME global
	end
	...
	local main = create("Frame", {
		Name = "Window", Parent = (not deferred) and gui or nil,
		...
		Visible = not deferred,
	})
	corner(main, RADIUS_LARGE)
	stroke(main, self._theme.Border, 1)

	-- ---- background image (optional) ---------------------------------
	-- opts.Background = "rbxassetid://..."  -> gambar jadi backdrop window
	-- opts.BackgroundTransparency (0..1) -> transparansi gambarnya (default 0)
	-- opts.Overlay (0..1) -> tint gelap di atas gambar biar teks kebaca (default 0.35)
	if opts.Background then
		-- biar gambar keliatan, frame utama dibikin transparan penuh
		main.BackgroundTransparency = 1
		local bg = create("ImageLabel", {
			Name = "BackgroundImage", Parent = main,
			BackgroundTransparency = 1,
			Image = opts.Background,
			ImageTransparency = opts.BackgroundTransparency or 0,
			ScaleType = Enum.ScaleType.Crop,
			Size = UDim2.new(1, 0, 1, 0),
			Position = UDim2.new(0, 0, 0, 0),
			ZIndex = 0,
		})
		corner(bg, RADIUS_LARGE)
		-- tint gelap di atas gambar (opsional) supaya elemen tetap kebaca
		local overlayT = opts.Overlay
		if overlayT == nil then overlayT = 0.35 end
		if overlayT < 1 then
			local ov = create("Frame", {
				Name = "BackgroundOverlay", Parent = main,
				BackgroundColor3 = self._theme.Background,
	...
	-- MINIMIZE -> floating logo (squircle / kotak pinggiran mulus) -----
	-- opts.Icon            -> gambar/glyph di logo (default huruf pertama title)
	-- opts.FloatIconRadius -> radius sudut logo. 0 = kotak tajam, makin gede makin bulat.
	--                         default 14 (squircle). Pakai 25 buat bulat penuh lagi.
	local floatRadius = opts.FloatIconRadius or 14
	local floatIcon = create("TextButton", {
		Name = "FloatIcon", Parent = gui, BackgroundColor3 = self._theme.Surface, BorderSizePixel = 0,
		Text = "", Font = FONT_TITLE, TextColor3 = self._theme.Accent, TextSize = 20,
		AutoButtonColor = false, Size = UDim2.fromOffset(50, 50), Position = UDim2.new(0, 20, 1, -70),
		Visible = false, Active = true, ZIndex = 400,
	})
	create("UICorner", { CornerRadius = UDim.new(0, floatRadius), Parent = floatIcon })
	stroke(floatIcon, self._theme.Accent, 1)
	addHover(floatIcon, self._theme.Surface, self._theme.SurfaceHover)
	-- isi logo: gambar kalau opts.Icon berupa image, glyph kalau icon terdaftar, else huruf pertama
	do
		local fri = resolveIcon(opts.Icon)
		if fri and fri.image then
			local fimg = create("ImageLabel", {
				Name = "FloatImage", Parent = floatIcon, BackgroundTransparency = 1,
				Image = fri.image, ScaleType = Enum.ScaleType.Fit, ZIndex = 401,
				AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0.5, 0),
				Size = UDim2.new(1, -14, 1, -14),
			})
			create("UICorner", { CornerRadius = UDim.new(0, math.max(0, floatRadius - 5)), Parent = fimg })
		elseif fri and fri.glyph then
			floatIcon.Text = fri.glyph
		else
			floatIcon.Text = string.sub(title, 1, 1):upper()
		end
	end
	do
		-- fDragging cuma TRUE kalau press-nya beneran mulai DI ATAS logo.
		-- deteksi lepas pakai InputEnded global yg di-gate fDragging, jadi klik
		-- di layar (yg gak pernah nge-set fDragging) TIDAK akan buka window.
		-- (dulu pakai input.Changed per-klik yg nyangkut + InputObject di-recycle
		--  Roblox -> klik layar ikut nge-trigger buka window. itu bug-nya.)
		...
		UserInputService.InputEnded:Connect(function(input)
			if fDragging and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
				fDragging = false
				-- cuma buka window kalau ini beneran klik (bukan drag)
...
-- Window:Show() -> reveal window yang dibuat dengan Deferred=true.
-- Dipakai bareng CreateLoadingScreen: panggil di dalam Loader:Finish(...).
function Window:Show()
	local main = self._main
	if not main then return self end
	if main.Visible and main.Parent then return self end
	-- semua gambar dipreload dulu (max 2 detik) biar ga pop-in pas muncul
	pcall(function() self._vypers:PreloadAssets(2) end)
	BUILDING = false
	if not main.Parent then main.Parent = self._gui end
	local TweenService = game:GetService("TweenService")
	local scale = main:FindFirstChildOfClass("UIScale") or Instance.new("UIScale")
	scale.Scale = 0.965
	scale.Parent = main
	if self._floatIcon then self._floatIcon.Visible = false end
	main.Visible = true
	local tw = TweenService:Create(scale, TweenInfo.new(0.24, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 1 })
	tw.Completed:Connect(function() if scale then scale:Destroy() end end)
	tw:Play()
	return self
end
...
function Window:CreateTab(opts)
	...
	local theme = self._vypers._theme
	local tabT = self._vypers._tabT or 0
	...
	local btn = create("TextButton", {
		...
		BackgroundTransparency = tabT, BorderSizePixel = 0,
	...
	local function activate()
		...
		for _, t in ipairs(self._tabs) do
			...
			t._btn.BackgroundTransparency = tabT
			t._btn.TextColor3 = theme.TextDim
		end
		page.Visible = true
		btn.BackgroundColor3 = theme.Accent
		btn.BackgroundTransparency = math.min(tabT, 0.15)
	...
	btn.MouseEnter:Connect(function()
		if self._activeTab ~= self_tab then
			...
			btn.BackgroundTransparency = tabT
		end
	end)
...
function Tab:CreateSection(opts)
	...
	local container = create("Frame", {
		...
		BackgroundColor3 = theme.Surface, BackgroundTransparency = boxed and (self._vypers._sectionT or 0) or 1, BorderSizePixel = 0,
...
function Section:_row(height)
	local row = create("Frame", {
		Parent = self._body, BackgroundColor3 = self._theme.SurfaceLight, BackgroundTransparency = self._vypers._surfaceT or 0, BorderSizePixel = 0,
...
-- =====================================================================
--  ELEMENT: DEVELOPER CARD (kartu profil developer biar keren)
-- =====================================================================
-- Contoh:
--   Section:CreateDeveloper({
--       Id      = "dev",
--       Name    = "Yeremia",
--       Caption = "Lead Developer - King Vypers",
--       Image   = "rbxassetid://139467646163013",   -- foto/avatar (assetid | url)
--       Accent  = Color3.fromRGB(120, 90, 240),      -- opsional (default theme.Accent)
--   })
function Section:CreateDeveloper(opts)
	opts = opts or {}
	local theme = self._theme
	local accent = opts.Accent or theme.Accent
	...
	local H = opts.Height or 74
	local card = create("Frame", {
		Parent = self._body, BackgroundColor3 = theme.SurfaceLight, BorderSizePixel = 0,
		Size = UDim2.new(1, 0, 0, H), LayoutOrder = self:_next(), ClipsDescendants = true,
	})
	corner(card, RADIUS_LARGE)
	stroke(card, theme.Border, 1)

	-- gradient accent halus di background biar keliatan keren
	local grad = create("Frame", {
		Parent = card, BackgroundColor3 = accent, BorderSizePixel = 0,
		Size = UDim2.new(1, 0, 1, 0), ZIndex = 1,
	})
	corner(grad, RADIUS_LARGE)
	create("UIGradient", {
		Parent = grad,
		Color = ColorSequence.new(accent, theme.SurfaceLight),
	...
	local avatarHolder = create("Frame", {
		Parent = card, BackgroundColor3 = theme.Background, BorderSizePixel = 0,
		AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 12, 0.5, 0),
		Size = UDim2.fromOffset(AV, AV), ZIndex = 2, ClipsDescendants = true,
	})
	create("UICorner", { Parent = avatarHolder, CornerRadius = UDim.new(1, 0) })
	stroke(avatarHolder, accent, 2)
	if opts.Image then
		local img = create("ImageLabel", {
			Parent = avatarHolder, BackgroundTransparency = 1, Image = toAsset(opts.Image),
			ScaleType = Enum.ScaleType.Crop, Size = UDim2.new(1, 0, 1, 0), ZIndex = 2,
		})
		create("UICorner", { Parent = img, CornerRadius = UDim.new(1, 0) })   -- bulet-in gambarnya langsung
	end

	-- nama + caption
	local textX = 12 + AV + 12
	local nameLbl = create("TextLabel", {
		Parent = card, BackgroundTransparency = 1, Font = FONT_TITLE, Text = opts.Name or "Developer",
		TextColor3 = theme.Text, TextSize = 15, TextXAlignment = Enum.TextXAlignment.Left,
		Position = UDim2.new(0, textX, 0, 15), Size = UDim2.new(1, -textX - 12, 0, 20), ZIndex = 2,
	})
	local capLbl = create("TextLabel", {
		Parent = card, BackgroundTransparency = 1, Font = FONT_VALUE, Text = opts.Caption or "",
		TextColor3 = theme.TextDim, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left,
		TextWrapped = true, Position = UDim2.new(0, textX, 0, 37), Size = UDim2.new(1, -textX - 12, 0, 28), ZIndex = 2,
	})

	local handle = {
		Type = "Developer", Id = opts.Id, Instance = card,
		SetName    = function(v) nameLbl.Text = tostring(v) end,
		SetCaption = function(v) capLbl.Text = tostring(v) end,
		SetImage   = function(v) if opts.Image then end end,
	}
	registerElement(opts.Id, handle)
	return handle
end
...
-- =====================================================================
--  ELEMENT: DEVELOPERS (daftar banyak developer, compact)
-- =====================================================================
-- Contoh:
--   Section:CreateDevelopers({
--       Title  = "Developers",                       -- opsional header
--       Accent = Color3.fromRGB(120, 90, 240),       -- opsional (default accent window)
--       AvatarSize = 30,                             -- opsional, gede-kecilnya avatar
--       List = {
--           { Name = "Yeremia", Role = "Lead Developer", Image = "rbxassetid://139467646163013" },
--           { Name = "Budi",    Role = "UI Designer",    Image = "rbxassetid://0000000000", Accent = Color3.fromRGB(80,190,120) },
--       },
--   })
function Section:CreateDevelopers(opts)
	opts = opts or {}
	local theme = self._theme
	local accent = opts.Accent or theme.Accent
	local list = opts.List or opts.Developers or {}
	local function toAsset(v)
		if type(v) == "number" then return "rbxassetid://" .. v end
		return tostring(v)
	end

	local AV = opts.AvatarSize or 30
	local ROW_H = AV + 6

	local card = create("Frame", {
		Parent = self._body, BackgroundColor3 = theme.SurfaceLight, BorderSizePixel = 0,
		Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
		LayoutOrder = self:_next(), ClipsDescendants = true,
	})
	corner(card, RADIUS_LARGE)
	stroke(card, theme.Border, 1)
	create("UIPadding", { Parent = card, PaddingTop = UDim.new(0, 8), PaddingBottom = UDim.new(0, 8),
		PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(0, 10) })
	create("UIListLayout", { Parent = card, Padding = UDim.new(0, 4), SortOrder = Enum.SortOrder.LayoutOrder })

	if opts.Title then
		create("TextLabel", {
			Parent = card, BackgroundTransparency = 1, Font = FONT_TITLE, Text = opts.Title,
			TextColor3 = theme.Text, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left,
			Size = UDim2.new(1, 0, 0, 18), LayoutOrder = 0,
		})
	end

	for i, dev in ipairs(list) do
		local devAccent = dev.Accent or accent
		local row = create("Frame", {
			Parent = card, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, ROW_H), LayoutOrder = i,
		})
		local holder = create("Frame", {
			Parent = row, BackgroundColor3 = theme.Background, BorderSizePixel = 0,
			AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 0, 0.5, 0),
			Size = UDim2.fromOffset(AV, AV), ClipsDescendants = true,
		})
		create("UICorner", { Parent = holder, CornerRadius = UDim.new(1, 0) })
		stroke(holder, devAccent, 2)
		if dev.Image then
			local img = create("ImageLabel", {
				Parent = holder, BackgroundTransparency = 1, Image = toAsset(dev.Image),
		...
		local tx = AV + 10
		create("TextLabel", {
			Parent = row, BackgroundTransparency = 1, Font = FONT_TITLE, Text = dev.Name or "Developer",
			TextColor3 = theme.Text, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left,
			Position = UDim2.new(0, tx, 0, 3), Size = UDim2.new(1, -tx, 0, 16),
		})
		create("TextLabel", {
			Parent = row, BackgroundTransparency = 1, Font = FONT_VALUE, Text = dev.Role or dev.Caption or "",
			TextColor3 = theme.TextDim, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left,
			Position = UDim2.new(0, tx, 0, 19), Size = UDim2.new(1, -tx, 0, 14),
		})
	end

	local handle = { Type = "Developers", Id = opts.Id, Instance = card }
	registerElement(opts.Id, handle)
	return handle
end
...
function Section:CreateParagraph(opts)
	...
	local imageObj
	if opts.Image then
		-- ImageScaleType: "Fit" (default, jaga rasio, bisa ada bar) | "Crop" (penuhin card, potong kelebihan) | "Stretch"
		local st = Enum.ScaleType.Fit
		if opts.ImageScaleType == "Crop" or opts.ImageFill then
			st = Enum.ScaleType.Crop
		elseif opts.ImageScaleType == "Stretch" then
			st = Enum.ScaleType.Stretch
		end
		imageObj = create("ImageLabel", {
			Parent = box, BackgroundColor3 = theme.Background, BorderSizePixel = 0, Image = toAsset(opts.Image),
			ScaleType = st, Size = UDim2.new(1, 0, 0, opts.ImageHeight or 140), LayoutOrder = 3, ClipsDescendants = true,
		})
...
-- =====================================================================
--  ELEMENT: BUTTON ROW (beberapa button bersebelahan kiri-kanan)
-- =====================================================================
-- Contoh:
--   Section:CreateButtonRow({ Buttons = {
--       { Title = "Load Config",  Callback = fn },
--       { Title = "Paste Config", Callback = fn, Color = Color3.fromRGB(80,120,220) },
--   }})
-- Lebar dibagi rata otomatis sesuai jumlah button.
function Section:CreateButtonRow(opts)
	opts = opts or {}
	local buttons = opts.Buttons or {}
	local n = math.max(#buttons, 1)
	local GAP = opts.Gap or 8

	local row = self:_row(opts.Height or ELEMENT_H)
	row.BackgroundTransparency = 1  -- biar cuma button-nya yang keliatan
	...
	local wOffset = -((n - 1) * GAP) / n
	local handles = {}
	for i, b in ipairs(buttons) do
		local baseColor  = b.Color or self._theme.SurfaceLight
		local textColor  = b.Color and pickText(b.Color) or self._theme.Text
		local hoverColor = b.Color and lighten(b.Color, 0.08) or self._theme.SurfaceHover

		local btn = create("TextButton", {
			Parent = row, BackgroundColor3 = baseColor, BorderSizePixel = 0,
			Font = FONT_TITLE, Text = b.Title or "Button", TextColor3 = textColor,
			TextSize = 13, AutoButtonColor = false,
			Size = UDim2.new(1 / n, wOffset, 1, 0), LayoutOrder = i,
		})
		corner(btn, RADIUS_LARGE)
		stroke(btn, self._theme.Border, 1)
		addHover(btn, baseColor, hoverColor)
...
function Section:CreateInput(opts)
	...
	local boxHolder = create("Frame", {
		...
		ClipsDescendants = true,  -- teks panjang di-clip biar tetap di dalam kotak
...
function Section:_buildDropdown(opts, multi)
	...
	if sidebarMode then
		...
		-- MULTI-only: tombol bantu Select All / Clear di header sidebar
		local listTop = 70
		if multi and opts.SelectAll ~= false then
			listTop = 100
			local ctrlRow = create("Frame", {
				Parent = sidebarPanel, BackgroundTransparency = 1,
				Position = UDim2.new(0, 10, 0, 68), Size = UDim2.new(1, -20, 0, 24), ZIndex = 302,
			})
			local function mkCtrl(txt, xScale, xOff)
				local b = create("TextButton", {
					Parent = ctrlRow, BackgroundColor3 = self._theme.Background, BorderSizePixel = 0,
					Font = FONT_VALUE, Text = txt, TextColor3 = self._theme.Text, TextSize = 11,
					AutoButtonColor = false, Position = UDim2.new(xScale, xOff, 0, 0),
					Size = UDim2.new(0.5, -4, 1, 0), ZIndex = 303,
				})
				corner(b, RADIUS_SMALL); stroke(b, self._theme.Border, 1)
				addHover(b, self._theme.Background, self._theme.SurfaceHover)
				return b
			end
			local selAllBtn = mkCtrl("Select All", 0, 0)
			local clearBtn  = mkCtrl("Clear", 0.5, 4)
			selAllBtn.MouseButton1Click:Connect(function()
				for _, v in ipairs(valueList) do selected[v] = true end
				refresh(); pcall(callback, selectionValue()); autoSaveTrigger()
			end)
			clearBtn.MouseButton1Click:Connect(function()
				if allowNone then
					for k in pairs(selected) do selected[k] = nil end
				else
					-- AllowNone=false: sisakan minimal 1 pilihan
					local first = valueList[1]
					for k in pairs(selected) do selected[k] = nil end
					if first ~= nil then selected[first] = true end
				end
				refresh(); pcall(callback, selectionValue()); autoSaveTrigger()
			end)
		end
		...
		listFrame.Position = UDim2.new(0, 10, 0, listTop)
		listFrame.Size = UDim2.new(1, -20, 1, -(listTop + 10))
...
-- =====================================================================
-- =====================================================================
--  BUILD SCHEDULER API  (anti nge-frame)
-- ---------------------------------------------------------------------
--  Vypers:SetBuildBudget(ms)   -> max ms kerja per frame saat build (default 4)
--  Vypers:Yield(force)         -> yield manual kalau budget kelewat
--  Vypers:PreloadAssets(sec)   -> preload semua image UI (dipanggil auto di Show)
-- =====================================================================
function Vypers:SetBuildBudget(ms)
	BUILD_BUDGET = math.clamp((tonumber(ms) or 4) / 1000, 0.001, 0.05)
	return BUILD_BUDGET
end

function Vypers:Yield(force)
	budgetYield(force)
	return self
end

function Vypers:PreloadAssets(maxWait)
	local list, seen = {}, {}
	for _, id in ipairs(PRELOAD) do
	...
	local deadline = os.clock() + (tonumber(maxWait) or 2)
	while not done and os.clock() < deadline do task.wait() end
	table.clear(PRELOAD)
	return self
end

-- Semua method Create* dibungkus biar otomatis nyicil antar frame selama build.
-- Jadi script pemakai GA PERLU nyebar task.wait() manual lagi.
do
	local function wrapCreators(proto)
		for name, fn in pairs(proto) do
			if type(fn) == "function" and string.sub(name, 1, 6) == "Create" then
				proto[name] = function(...)
					if BUILDING then budgetYield() end
					return fn(...)
				end
			end
		end
...
-- =====================================================================
--  STREAM PROOF  (Hide from Record & Stream)
-- ---------------------------------------------------------------------
--  Vypers:SetStreamProof(state, opts)  -> nyalain/matiin mode stream proof
--        opts = { Native = true, HideOnScreenshot = true, HideOnUnfocus = false }
--  Vypers:GetStreamProof()             -> { Enabled, Native, NativeName, Methods, Supported }
--  Vypers:IsStreamProofSupported()     -> executor punya native anti-capture?
--  Vypers:SetUIVisible(bool) / :IsUIVisible() / :ToggleUIVisible()
--  Vypers:PanicHide(bool)              -> sembunyiin UI manual (hotkey panic)
--  Vypers:FlashHide(seconds)           -> sembunyiin sebentar (anti screenshot)
-- =====================================================================
local SP = {
	Enabled          = false,
	NativeApplied    = false,
...
local WDA_NONE               = 0
local WDA_EXCLUDEFROMCAPTURE = 17 -- 0x11

local SP_NATIVE_NAMES = {
	"set_window_display_affinity", "setwindowdisplayaffinity", "SetWindowDisplayAffinity",
	"set_display_affinity", "setdisplayaffinity", "hide_from_capture", "hidefromcapture",
	"set_stream_proof", "setstreamproof", "streamproof", "toggle_streamproof",
	"protect_window", "protectwindow",
}
local SP_NAMESPACES = { "syn", "fluxus", "wave", "krnl", "delta", "solara", "swift", "executor" }

local function spEnv()
	local ok, env = pcall(function() return (getgenv and getgenv()) or _G end)
	if ok and type(env) == "table" then return env end
	return _G
end

local function spGetFn(container, name)
	local ok, v = pcall(function() return container[name] end)
	if ok and type(v) == "function" then return v end
	return nil
end

local function spFindNative()
	local env = spEnv()
	for _, n in ipairs(SP_NATIVE_NAMES) do
		local fn = spGetFn(env, n) or spGetFn(_G, n)
		if fn then return fn, n end
	end
	for _, ns in ipairs(SP_NAMESPACES) do
		local ok, tbl = pcall(function() return env[ns] end)
		if ok and type(tbl) == "table" then
			for _, n in ipairs(SP_NATIVE_NAMES) do
				local fn = spGetFn(tbl, n)
				if fn then return fn, ns .. "." .. n end
			end
		end
	end
	return nil, nil
...
local function spApplyNative(state)
	local fn, name = spFindNative()
	if not fn then return false, nil end
	local affinity = state and WDA_EXCLUDEFROMCAPTURE or WDA_NONE
	local tries = { { state }, { affinity }, { state and 1 or 0 } }
	for _, args in ipairs(tries) do
		local ok = pcall(function() return fn(table.unpack(args)) end)
		if ok then return true, name end
	end
	return false, name
end

local function spCaptureService()
	local ok, svc = pcall(function() return game:GetService("CaptureService") end)
	if ok and typeof(svc) == "Instance" then return svc end
	return nil
end

local function spClearConns()
	for _, c in ipairs(SP._conns) do pcall(function() c:Disconnect() end) end
	table.clear(SP._conns)
end

function Vypers:SetUIVisible(state)
	state = state and true or false
	local gui = self._gui
	if gui then pcall(function() gui.Enabled = state end) end
	return self
end

function Vypers:IsUIVisible()
	local gui = self._gui
...
function Vypers:ToggleUIVisible()
	local vis = not self:IsUIVisible()
	self:SetUIVisible(vis)
	SP._userHidden = not vis
	return vis
end

function Vypers:PanicHide(state)
	state = state and true or false
	SP._userHidden = state
	self:SetUIVisible(not state)
	return state
end

function Vypers:FlashHide(seconds)
	if SP._busy then return self end
	if not self:IsUIVisible() then return self end
	SP._busy = true
	self:SetUIVisible(false)
	task.delay(tonumber(seconds) or 1.5, function()
		SP._busy = false
		if not SP._userHidden then Vypers:SetUIVisible(true) end
	end)
	return self
end

function Vypers:IsStreamProofSupported()
	local fn, name = spFindNative()
	return fn ~= nil, name
end

function Vypers:GetStreamProof()
	local supported = (spFindNative() ~= nil)
	local methods = {}
	for i, m in ipairs(SP.Methods) do methods[i] = m end
	return {
		Enabled          = SP.Enabled,
		Native           = SP.NativeApplied,
		NativeName       = SP.NativeName,
		Methods          = methods,
		Supported        = supported,
		HideOnScreenshot = SP.HideOnScreenshot,
		HideOnUnfocus    = SP.HideOnUnfocus,
		Hidden           = SP._userHidden,
	}
end

function Vypers:SetStreamProof(state, opts)
	opts = opts or {}
	state = state and true or false
	if opts.HideOnScreenshot ~= nil then SP.HideOnScreenshot = opts.HideOnScreenshot and true or false end
	if opts.HideOnUnfocus ~= nil then SP.HideOnUnfocus = opts.HideOnUnfocus and true or false end
	SP.Enabled = state
	SP.Methods = {}
	spClearConns()

	-- LAYER 1: native executor anti-capture (WDA_EXCLUDEFROMCAPTURE)
	local nativeOk, nativeName = false, nil
	if opts.Native ~= false then nativeOk, nativeName = spApplyNative(state) end
	SP.NativeApplied, SP.NativeName = nativeOk, nativeName
	if nativeOk then table.insert(SP.Methods, "native (" .. tostring(nativeName) .. ")") end

	if not state then
		if not SP._userHidden then self:SetUIVisible(true) end
		return self:GetStreamProof()
	end

	-- LAYER 2: GUI ditaruh di container tersembunyi (ga kebaca game / screenshare explorer)
	pcall(function()
		local gui = self._gui
		if gui and typeof(gethui) == "function" then
			local hidden = gethui()
			if hidden and gui.Parent ~= hidden then gui.Parent = hidden end
		end
	end)
	pcall(function()
		local env = spEnv()
		if type(env.syn) == "table" and type(env.syn.protect_gui) == "function" and self._gui then
			env.syn.protect_gui(self._gui)
		end
	end)
	table.insert(SP.Methods, "protected gui")

	-- LAYER 3: CaptureService (screenshot / record bawaan Roblox) -> UI auto hilang
	local cs = spCaptureService()
	if cs then
		local hooked = false
		pcall(function()
			if cs.CaptureBegan then
				table.insert(SP._conns, cs.CaptureBegan:Connect(function()
					Vypers:SetUIVisible(false)
				end))
				hooked = true
			end
		end)
		pcall(function()
			if cs.CaptureEnded then
				table.insert(SP._conns, cs.CaptureEnded:Connect(function()
		...
		if hooked then table.insert(SP.Methods, "capture service") end
	end

	-- LAYER 4: screenshot guard (PrintScreen / F12 / Win+Shift+S / Ctrl+Shift+S)
	if SP.HideOnScreenshot then
		table.insert(SP._conns, UserInputService.InputBegan:Connect(function(input)
			if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
			local kc = input.KeyCode
			local shift = UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift)
			local ctrl  = UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.RightControl)
			local meta  = UserInputService:IsKeyDown(Enum.KeyCode.LeftMeta) or UserInputService:IsKeyDown(Enum.KeyCode.RightMeta)
			local hit = (kc == Enum.KeyCode.Print)
				or (kc == Enum.KeyCode.F12)
				or (kc == Enum.KeyCode.S and shift and (meta or ctrl))
			if hit then Vypers:FlashHide(2) end
		end))
		table.insert(SP.Methods, "screenshot guard")
	end

	-- LAYER 5: auto hide pas window Roblox ga fokus (alt-tab ke OBS / Discord)
	if SP.HideOnUnfocus then
		table.insert(SP._conns, UserInputService.WindowFocusReleased:Connect(function()
			Vypers:SetUIVisible(false)
		end))
		table.insert(SP._conns, UserInputService.WindowFocused:Connect(function()
			if not SP._userHidden then Vypers:SetUIVisible(true) end
		end))
		table.insert(SP.Methods, "focus guard")
	end

	return self:GetStreamProof()
end

	wrapCreators(Section)
	wrapCreators(Tab)
	wrapCreators(Window)
end
