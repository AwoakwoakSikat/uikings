...
function Vypers:CreateWindow(opts)
	...
	-- topbar: tombol CLOSE dihapus, diganti tombol HIDE.
	-- HIDE = window + float icon ilang semua. Munculin lagi: klik lokasi
	-- terakhir float icon (ada hotspot invisible di posisi itu).
	local hideBtn = makeCtrlButton("\u{25CE}", -30, Color3.fromRGB(200, 60, 60))
	...
	-- ---- HIDE (ganti tombol close) -------------------------------------
	-- opts.HideRestore = "window" (default) -> klik hotspot langsung buka window
	--                  = "icon"            -> klik hotspot balikin float icon dulu
	local restoreMode = (opts.HideRestore == "icon") and "icon" or "window"
	local ghost = create("TextButton", {
		Name = "HideSpot", Parent = gui, BackgroundTransparency = 1, Text = "",
		AutoButtonColor = false, Size = floatIcon.Size, Position = floatIcon.Position,
		Visible = false, Active = true, ZIndex = 500,
	})
	local hiddenFrom = nil -- "window" | "icon"

	local function doHide()
		if activeDropdownClose then activeDropdownClose() end
		if hiddenFrom then return end
		hiddenFrom = main.Visible and "window" or "icon"
		-- inget posisi + ukuran float icon terakhir, hotspot ditaruh persis di situ
		ghost.Size = floatIcon.Size
		ghost.Position = floatIcon.Position
		main.Visible = false
	...
	local function doUnhide()
		...
		ghost.Visible = false
		local back = (hiddenFrom == "icon") and "icon" or restoreMode
		hiddenFrom = nil
		if back == "icon" then
			main.Visible = false
	...
	ghost.MouseButton1Click:Connect(doUnhide)
	hideBtn.MouseButton1Click:Connect(doHide)

	self_win._ghost    = ghost
	self_win._doHide   = doHide
	self_win._doUnhide = doUnhide
	self_win._isHidden = function() return hiddenFrom ~= nil end
...
function Window:Toggle()
	-- kalau lagi ke-hide (semua ilang), toggle key = munculin lagi
	if self._isHidden and self._isHidden() then
		if self._doUnhide then self._doUnhide() end
		return
	end
...
-- Window:Hide()     -> window + float icon ilang semua (hotspot ditinggal)
-- Window:Unhide()   -> munculin lagi
-- Window:IsHidden() -> lagi ke-hide atau ga
function Window:Hide()
	if self._doHide then self._doHide() end
	return self
end
...
function Window:IsHidden()
	return (self._isHidden and self._isHidden()) or false
end
...
function Window:Show()
	...
	if self._ghost then self._ghost.Visible = false end
	if self._doUnhide and self._isHidden and self._isHidden() then self._doUnhide() end
	main.Visible = true
