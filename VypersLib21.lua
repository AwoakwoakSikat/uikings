...
function Section:_buildDropdown(opts, multi)
	...
	local open = false
	local slideConn
	-- base (collapsed) row height, captured lazily on first open so it also works
	-- when opts.Desc grew the row a little.
	local baseRowH
	local function listHeight() return math.min(contentHeight, MAX_H) end
	-- INLINE MODE (non-sidebar): the option list drops DOWN inside the row and
	-- grows the row height, pushing the rows below it down -- it is NOT a floating
	-- overlay popup anymore. Only this dropdown's own row expands.
	local function positionList()
		if sidebarMode then return end
		if not baseRowH then baseRowH = row.Size.Y.Offset end
		local h = listHeight()
		listFrame.Position = UDim2.new(0.4, 4, 0, baseRowH + 2)
		listFrame.Size = UDim2.new(0.6, -14, 0, h)
		if open then
			row.Size = UDim2.new(1, 0, 0, baseRowH + h + 6)
		end
	end
	local function closeList()
		...
		if sidebarMode and sidebarPanel then
			...
			-- collapse the row back down, then hide the list once shrunk
			if not baseRowH then baseRowH = row.Size.Y.Offset end
			row:TweenSize(UDim2.new(1, 0, 0, baseRowH), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.15, true, function()
	...
	local function openList()
		...
		-- inline expand: clip the row so the list slides open cleanly
		...
		listFrame.Size = UDim2.new(0.6, -14, 0, h)
		listFrame.Visible = true
		row:TweenSize(UDim2.new(1, 0, 0, baseRowH + h + 6), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.15, true)
		activeDropdownClose = closeList
	end
