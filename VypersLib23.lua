...
function Section:_buildDropdown(opts, multi)
	...
	local function positionList()
		...
		-- anchor the list to the FIELD (selBtn) itself, so it drops straight DOWN
		-- from right under the clicked "Balanced" box (not under the description)
		local fx = selBtn.AbsolutePosition.X
		local fy = selBtn.AbsolutePosition.Y
		local fw = selBtn.AbsoluteSize.X
		local fh = selBtn.AbsoluteSize.Y
		local y = fy + fh + 3
		local vpY = gui.AbsoluteSize.Y
		if y + listH > vpY - 6 and (fy - listH - 3) >= 6 then
			y = fy - listH - 3
		end
		listFrame.Position = UDim2.fromOffset(fx, y)
