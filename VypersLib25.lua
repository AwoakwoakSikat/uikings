...
function Section:_buildDropdown(opts, multi)
	...
	local open = false
	local posConn
	-- NORMAL (popup) dropdown: a floating list that drops straight DOWN from the
	-- clicked dropdown's OWN row, re-glued every frame so it never drifts. It may
	-- overlap the rows below -- that's fine, it's a popup. SIDEBAR mode still uses
	-- the right-side drawer.
	local function positionList()
		if sidebarMode then return end
		local listH = math.min(contentHeight, MAX_H)
		-- Compute the field position RELATIVE to the ScreenGui origin (subtract
		-- gui.AbsolutePosition) so it stays correct whether or not the topbar GUI
		-- inset is applied. The list ALWAYS drops straight down from just under the
		-- clicked field -- no flipping, so it can never cover the field.
		local origin = gui.AbsolutePosition
		local fx = selBtn.AbsolutePosition.X - origin.X
		local fy = selBtn.AbsolutePosition.Y - origin.Y
		local fw = selBtn.AbsoluteSize.X
		local fh = selBtn.AbsoluteSize.Y
		local top = fy + fh + 3
		-- keep the list INSIDE the window: clamp its height to the window's bottom
		-- edge so it SCROLLS instead of spilling out into the game world
		local win = self._window and self._window._main
		if win then
			local winBottom = (win.AbsolutePosition.Y - origin.Y) + win.AbsoluteSize.Y
			local avail = winBottom - top - 8
			listH = math.min(listH, math.max(avail, 48))
		end
	...
	local function openList()
		...
		if sidebarMode and sidebarPanel then
			...
			posConn = UserInputService.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
